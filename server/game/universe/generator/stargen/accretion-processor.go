...
func randEccentricity() float64 {
	E := 1.0 - math.Pow(utils.RandFloat64(0.0, 1.0), EccentricityCoefficient)
	if E > 0.9999 {
		E = 0.999
	}
	return E
...
func (accretionProcessor *AccretionProcessor) isDustAvailable(insideRange, outsideRange float64) bool {
	...
	dustBandElement := container.Find(accretionProcessor.DustBands, func(v interface{}, index int64) bool {
	...
	dustBandElement = container.FindAfter(dustBandElement, func(v interface{}) bool {
		dustBand := v.(*DustBand)
...
func (accretionProcessor *AccretionProcessor) updateDustLanes(min, max, mass, critMass, bodyInnerBound, bodyOuterBound float64) bool {
	...
	container.ForEachElement(accretionProcessor.DustBands, func(element *list.Element, index int64) {
...
func (accretionProcessor *AccretionProcessor) DistPlanetaryMasses(stellarMassRatio, stellarLuminosityRatio, innerDust, outerDust, outerPlanetLimit, dustDensityCoefficient float64, doMoons bool) {
	...
	for dustLeft {
		...
		if accretionProcessor.isDustAvailable(innerEffectLimit, outerEffectLimit) {
			dustDensity := dustDensityCoefficient * math.Sqrt(stellarMassRatio) * math.Exp(-Alpha*math.Pow(a, (1.0/N)))
